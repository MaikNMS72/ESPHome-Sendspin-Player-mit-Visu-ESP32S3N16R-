...
void SendspinHub::setup() {
  ...
  sendspin::VisualizerRoleConfig visualizer_config;
  visualizer_config.support.types = {sendspin::VisualizerDataType::LOUDNESS, sendspin::VisualizerDataType::PEAK};
  visualizer_config.support.buffer_capacity = 4096;
  visualizer_config.support.rate_max = 10;
  visualizer_config.psram_stack = this->task_stack_in_psram_;
  this->visualizer_role_ = &this->client_->add_visualizer(std::move(visualizer_config));
...
#ifdef USE_SENDSPIN_VISUALIZER
  ESP_LOGCONFIG(TAG, "  Visualizer: loudness + peak, 10 fps");
#endif
}
...
#ifdef USE_SENDSPIN_VISUALIZER
// THREAD CONTEXT: Sendspin visualizer drain thread.
void SendspinHub::on_loudness(int64_t client_timestamp, uint16_t loudness) {
  (void) client_timestamp;
  this->visualizer_loudness_.store(loudness, std::memory_order_relaxed);
}

// THREAD CONTEXT: Sendspin visualizer drain thread.
void SendspinHub::on_peak(int64_t client_timestamp, uint8_t strength) {
  (void) client_timestamp;
  auto current = this->visualizer_peak_.load(std::memory_order_relaxed);
  while (strength > current &&
         !this->visualizer_peak_.compare_exchange_weak(current, strength, std::memory_order_relaxed)) {
  }
}
...
void SendspinHub::on_visualizer_stream_start(const sendspin::ServerVisualizerStreamObject &stream) {
  (void) stream;
  this->visualizer_active_.store(true, std::memory_order_relaxed);
  this->visualizer_loudness_.store(0, std::memory_order_relaxed);
  this->visualizer_peak_.store(0, std::memory_order_relaxed);
}

// THREAD CONTEXT: Main loop.
void SendspinHub::on_visualizer_stream_end() {
  this->visualizer_active_.store(false, std::memory_order_relaxed);
  this->visualizer_loudness_.store(0, std::memory_order_relaxed);
  this->visualizer_peak_.store(0, std::memory_order_relaxed);
}

// THREAD CONTEXT: Main loop.
void SendspinHub::on_visualizer_stream_clear() {
  this->visualizer_loudness_.store(0, std::memory_order_relaxed);
  this->visualizer_peak_.store(0, std::memory_order_relaxed);
}
#endif
