...
void SendspinMediaPlayer::setup() {
    ...
    if (group_obj.playback_state.has_value()) {
      ...
      switch (group_obj.playback_state.value()) {
        case sendspin::SendspinPlaybackState::PLAYING:
          this->powered_off_ = false;
          new_state = media_player::MEDIA_PLAYER_STATE_PLAYING;
          break;
        case sendspin::SendspinPlaybackState::STOPPED:
        default:
          new_state = this->powered_off_ ? media_player::MEDIA_PLAYER_STATE_OFF : media_player::MEDIA_PLAYER_STATE_IDLE;
          break;
      }
  ...
  this->parent_->add_controller_state_clear_callback(
      [this]() {
        this->set_playback_state_(this->powered_off_ ? media_player::MEDIA_PLAYER_STATE_OFF
...
void SendspinMediaPlayer::control(const media_player::MediaPlayerCall &call) {
  ...
  switch (command.value()) {
    ...
    case media_player::MEDIA_PLAYER_COMMAND_TURN_ON:
      this->powered_off_ = false;
      this->set_playback_state_(media_player::MEDIA_PLAYER_STATE_ON);
      this->parent_->send_client_command(sendspin::SendspinControllerCommand::PLAY);
      break;
    ...
    case media_player::MEDIA_PLAYER_COMMAND_TURN_OFF:
      this->powered_off_ = true;
      this->set_playback_state_(media_player::MEDIA_PLAYER_STATE_OFF);
      this->parent_->send_client_command(sendspin::SendspinControllerCommand::STOP);
      break;
