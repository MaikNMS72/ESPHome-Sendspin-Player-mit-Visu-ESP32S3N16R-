...
class SendspinHub final : public Component,
...
#ifdef USE_SENDSPIN_VISUALIZER
  uint16_t get_visualizer_loudness() const { return this->visualizer_loudness_.load(std::memory_order_relaxed); }
  uint8_t consume_visualizer_peak() { return this->visualizer_peak_.exchange(0, std::memory_order_relaxed); }
  bool is_visualizer_active() const { return this->visualizer_active_.load(std::memory_order_relaxed); }
#endif
 ...
 protected:
  ...
  std::atomic<uint16_t> visualizer_loudness_{0};
  std::atomic<uint8_t> visualizer_peak_{0};
  std::atomic<bool> visualizer_active_{false};
#endif
