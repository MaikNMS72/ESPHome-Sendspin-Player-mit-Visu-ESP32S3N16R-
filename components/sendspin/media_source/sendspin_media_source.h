...
class SendspinMediaSource final : public SendspinChild,
 ...
 public:
  ...
  float get_audio_level() const { return this->audio_level_.load(std::memory_order_relaxed) / 32768.0f; }
  float consume_audio_peak() { return this->audio_peak_.exchange(0, std::memory_order_relaxed) / 32768.0f; }
 ...
 protected:
  ...
  std::atomic<uint16_t> audio_level_{0};
  std::atomic<uint16_t> audio_peak_{0};
};
