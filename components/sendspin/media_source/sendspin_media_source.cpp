...
size_t SendspinMediaSource::on_audio_write(uint8_t *data, size_t length, uint32_t timeout_ms) {
  ...
  if (*params.bit_depth == 16 && length >= 2) {
    const auto *samples = reinterpret_cast<const int16_t *>(data);
    size_t sample_count = length / sizeof(int16_t);
    size_t step = std::max<size_t>(1, sample_count / 96);
    uint16_t peak = 0;
    uint32_t sum = 0;
    uint32_t counted = 0;

    for (size_t i = 0; i < sample_count; i += step) {
      int32_t sample = samples[i];
      uint16_t magnitude = static_cast<uint16_t>(std::min<int32_t>(32767, std::abs(sample)));
    ...
    if (counted > 0) {
      uint16_t average = static_cast<uint16_t>(sum / counted);
      uint16_t current = this->audio_peak_.load(std::memory_order_relaxed);
      while (peak > current && !this->audio_peak_.compare_exchange_weak(current, peak, std::memory_order_relaxed)) {
      }
      this->audio_level_.store(static_cast<uint16_t>(std::max<uint16_t>(average, peak / 3)), std::memory_order_relaxed);
    }
  }
